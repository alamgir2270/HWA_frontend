  ...
  const saveProfileChanges = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        insurance_info: profileForm.insurance_info,
      ...
      const response = await fetch(`${API_BASE}/api/patients/${profileForm.patient_id}`, {
        method: "PATCH",
        headers,
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          full_name: data.data?.User?.full_name || user.full_name,
                  ...
                  {!editingProfile ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      ...
                      <div>
                        ...
                        <input type="email" value={profileForm.email || ""} onChange={(e) => setProfileForm({ ...profileForm, email: e.target.value })} placeholder="you@example.com" style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid #ddd" }} />
                      </div>
                      <div>
                      ...
                      <div style={{ marginTop: 20, padding: 16, borderRadius: 8, backgroundColor: "#fafafa", border: "1px solid #e0e0e0" }}>
                        <h3 style={{ margin: "0 0 1rem 0" }}>Change Password</h3>
                        <p style={{ margin: "0 0 1rem 0", color: "#666" }}>Leave blank to keep your current password.</p>
                        <div style={{ display: "grid", gap: 10 }}>
                          <div>
                            <label style={{ display: "block", marginBottom: 4, fontWeight: "bold" }}>Current Password</label>
                            <input type="password" value={passwordForm.currentPassword} onChange={(e) => setPasswordForm({ ...passwordForm, currentPassword: e.target.value })} placeholder="Current password" style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid #ddd" }} />
                          </div>
                          <div>
                            <label style={{ display: "block", marginBottom: 4, fontWeight: "bold" }}>New Password</label>
                            <input type="password" value={passwordForm.newPassword} onChange={(e) => setPasswordForm({ ...passwordForm, newPassword: e.target.value })} placeholder="New password" style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid #ddd" }} />
                          </div>
                          <div>
                            <label style={{ display: "block", marginBottom: 4, fontWeight: "bold" }}>Confirm New Password</label>
                            <input type="password" value={passwordForm.confirmPassword} onChange={(e) => setPasswordForm({ ...passwordForm, confirmPassword: e.target.value })} placeholder="Confirm new password" style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid #ddd" }} />
                          </div>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={saveProfileChanges} style={{ padding: "0.5rem 1rem", backgroundColor: "#4caf50", color: "white", border: "none", borderRadius: 6, cursor: "pointer" }}>✓ Save</button>
...
const AppointmentCard = ({ appointment }) => (
  <div style={styles.itemCard}>
    <div style={styles.itemHeader}>
      <div>
        <h3 style={styles.itemTitle}>Dr. {getDoctorName(appointment)}</h3>
...
const BillCard = ({ bill, onPay }) => {
  ...
  const canPay = bill.payment_status !== "paid" && balanceDue > 0;
  ...
  <div style={styles.itemCard}>
    <div style={styles.itemHeader}>
      <div>
        ...
        <p style={styles.itemMeta}>Amount: BDT {totalAmount.toFixed(2)}</p>
        <p style={styles.itemMeta}>Paid: BDT {paidAmount.toFixed(2)}</p>
        <p style={styles.itemMeta} style={{color: balanceDue === 0 ? "#2ecc71" : "#e74c3c", fontWeight: "bold"}}>
          Balance Due: BDT {balanceDue.toFixed(2)}
        </p>
      </div>
